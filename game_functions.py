...
def change_fleet_direction(ai_settings, aliens):
    """Опускает весь флот вниз и меняет направление движения флота."""
    for alien in aliens.sprites():
...
def check_events(ai_settings, screen, stats, sb, play_button, ship, aliens,
                 bullets):
    """Обрабатывает нажанитя клавиш и события мыши."""
...
def update_screen(ai_settings, screen, stats, sb, ship, aliens, bullets,
    ...
    # Кнопка Play Показывается если игра не активна
...
def check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship,
    ...
    # Проверка на уничтожение флота
    if len(aliens) == 0:
        #  Удаление пуль и создание нового флота
        bullets.empty()
...
def get_number_rows(ai_settings, ship_height, alien_height):
    """Определяет количество рядов, помущающихся на экране."""
...
def ship_hit(ai_settings, stats, sb, screen, ship, aliens, bullets):
    ...
    # Сброс флага активного состояния игры
    else:
...
def check_aliens_bottom(ai_settings, stats, sb, screen, ship, aliens, bullets):
    ...
    for alien in aliens.sprites():
        if alien.rect.bottom >= screen_rect.bottom:
            # Вызов функции столкновения пришельца с кораблём
